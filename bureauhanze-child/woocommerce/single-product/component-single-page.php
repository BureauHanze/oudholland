	...
	<div class="component_inner" <?php echo in_array( 'toggled', $component_classes ) && in_array( 'closed', $component_classes ) ? 'style="display:none;"' : ''; ?>>
		<div class="component_description_wrapper"><?php
			...
			if ( $component->get_description() !== '' ) {
				...
				<div onclick="openPopup('<?php echo $title ?>')"  class="fieldInformation__button">
					Informatie <?php get_template_part('/assets/svg/icon-information'); ?>
				</div>

				<div id="<?php echo $title ?>" class="fieldInformation__content">
					<div class="fieldInformation__bg" onclick="closePopup('<?php echo $title ?>')"></div>
					<div class="fieldInformation__wrapper">
						<span onclick="closePopup('<?php echo $title ?>')" class="close">X</span>						
